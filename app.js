...
function renderModes(main) {
  ...
  // Timed Exam: available now for axial; reserved for appendicular/torso
  if (sec.exams && sec.exams.length) {
    modes.push({ id: "examPicker", icon: "⏱️", label: "Timed Exam", desc: `${sec.exams.length} practice exams · 30 s/question · keyboard shortcuts 1–5` });
  } else if (["appendicular", "torso"].includes(state.sectionKey)) {
    modes.push({ id: "_locked", icon: "🔒", label: "Timed Exam", desc: "Coming soon — practice exams being prepared for this section" });
  }
