...
function renderLabelingItem(main, ex) {
  if (lblItemIndex !== wsIndex) {
    lblItemIndex = wsIndex;
    lblAssignments = {};
  ...
  const stem = document.createElement("div");
  stem.className = "qStem";
  stem.textContent = `${wsIndex + 1}. ${ex.title}`;
  main.appendChild(stem);
  ...
  if (!lblChecked) {
    ...
    skip.onclick = () => {
      wsSkippedPoints += ex.blanks.length;
      wsIndex++;
      lblItemIndex = -1; lblAssignments = {}; lblSelectedChip = null; lblChecked = false;
      render();
    };
    ...
    const next = document.createElement("button");
    next.className = "nextBtn";
    next.textContent = wsIndex === wsDeck.length - 1 ? "See results" : "Next";
    next.onclick = () => { wsIndex++; render(); };
    fb.appendChild(next);
  }
  main.appendChild(fb);
}
